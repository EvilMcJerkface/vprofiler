...
#							Each id maps to a row containing
#							how much the transaction overlapped with each function, along with the
#							queueing time and the execution time of that transaction
def NonTargetCriticalPathBreak(criticalPaths, functionLog_dir, functionName_path):
	...
	for item in os.listdir(functionLog_dir):
		if os.path.isfile(os.path.join(functionLog_dir, item)) and re.match("FunctionLog", item):
	...
	# Iterate through every critical path, every interval in that path, every log file, and every row in that log file
	for txid, cpath in criticalPaths.items():
			...
			for log_path in functionLogPaths:
				...
				with open(log_path) as log_csv:
					...
					for row in log:
						...
						# then add the overlap time to the transaction's row in the function's column
		...
		# Attach the queueing time and the execution time to the row, then append it to the final result
