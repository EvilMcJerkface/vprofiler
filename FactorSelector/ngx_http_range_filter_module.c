...
ngx_http_range_filter_ctx_t  *ctx;
    ...
    if (in == NULL) {
        TRACE_START();
        resultReturn = ngx_http_next_body_filter(r, in);
        TRACE_END(1);
    ...
    if (ctx->ranges.nelts == 1) {
        TRACE_START();
        resultReturn = ngx_http_range_singlepart_body(r, ctx, in);
        TRACE_END(4);
    ...
    if (TRACE_S_E( ngx_buf_special(in->buf) ,5)) {
        TRACE_START();
        resultReturn = ngx_http_next_body_filter(r, in);
        TRACE_END(6);
    ...
    if (TRACE_S_E( ngx_http_range_test_overlapped(r, ctx, in) != NGX_OK ,7)) {
        TRACE_FUNCTION_END();
        return NGX_ERROR;
    }

    TRACE_START();
    resultReturn = ngx_http_range_multipart_body(r, ctx, in);
    TRACE_END(8);
